...
def center_window(self, w, h): #pass in the tkinter frame(master) reference and the w and h
    #get user's sceen width and height
...
def create_db(self):
    ...
    with conn:
        ...
        # Must commit to save changes and close connection
        conn.commit()
    conn.close()
    first_run(self)
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # forceuser to provide all info
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            # check db for existence of fullname, if exists, alert user and disregard request
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) #function to clear all textboxes and selected index of listbox
####                onRefersh(self)
                conn.commit()
        else:
...
def onRefresh(self):
    # Populate lisbox, coinciding with the database
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            cur = conn.cursor()
            # count records to see if user's changes arre already in the db...
            #meaning no changes to update
            ...
            print(count2)
            if count == 0 or count2 == 0: #if proposed changes are not already in the db, proceed
